...
const Hero = ({ layoutMode, content, setContent, editMode }) => {

    // Fallback to static props if content state isn't wired yet
    const heroContent = content?.hero || {
        heading: "Build Products That Scale With Your Vision.",
        subheading: "Transforming complex workflows into elegant, user-centric solutions.",
    ...
    if (layoutMode === 'dashboard') {
        const dashHeading = heroContent.heading === "Your AI Website" ? "Analytics Dashboard" : heroContent.heading;
        const dashSubheading = heroContent.subheading === "Generated intelligently from intent" ? "Monitor your key performance indicators, track active user sessions, and identify emerging trends instantly." : heroContent.subheading;
        ...
        return (
            <section className="animate-fade-in-up py-10 px-4 md:px-8 bg-transparent flex flex-col md:flex-row items-center justify-between gap-6 w-full relative z-10">
                <div>
                    <h1
                        className={`text-2xl md:text-3xl font-bold mb-2 tracking-tight ${editableClass}`}
                        contentEditable={editMode}
                        suppressContentEditableWarning={true}
                        onBlur={(e) => handleUpdate('heading', e)}
                    >
                        {dashHeading}
                    </h1>
                    <p
                        className={`text-sm opacity-70 max-w-lg ${editableClass}`}
                        contentEditable={editMode}
                        suppressContentEditableWarning={true}
                        onBlur={(e) => handleUpdate('subheading', e)}
                    >
                        {dashSubheading}
                    </p>
                </div>
    ...
    return (
        <section className="animate-fade-in-up py-28 px-6 md:px-12 w-full flex flex-col items-center justify-center text-center relative overflow-hidden bg-gradient-to-br from-blue-50/20 to-indigo-100/20 dark:from-blue-900/20 dark:to-indigo-900/20">
            ...
            <div className="relative z-10 max-w-4xl mx-auto flex flex-col items-center">
                ...
                <h1
                    className={`text-5xl md:text-7xl font-extrabold mb-8 tracking-tighter leading-tight ${editableClass}`}
                    contentEditable={editMode}
                    suppressContentEditableWarning={true}
                    onBlur={(e) => handleUpdate('heading', e)}
                >
                ...
                <p
                    className={`text-lg md:text-2xl font-light opacity-80 mb-12 max-w-3xl mx-auto leading-relaxed ${editableClass}`}
                    contentEditable={editMode}
                    suppressContentEditableWarning={true}
                    onBlur={(e) => handleUpdate('subheading', e)}
                >
                ...
                <div className="flex flex-wrap justify-center gap-5 w-full mt-4">
                    <button
                        className={`px-10 py-4 bg-blue-600 text-white font-semibold rounded-full hover:bg-blue-700 transition shadow-xl shadow-blue-500/20 hover:-translate-y-1 ${editMode ? 'border-dashed border-2 border-white/50 bg-blue-500' : ''}`}
                        contentEditable={editMode}
                        suppressContentEditableWarning={true}
                        onBlur={(e) => handleUpdate('cta', e)}
                    >
                        {heroContent.cta}
                    </button>
